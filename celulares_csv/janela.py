...
class Aplicacao():
    ...
    def frames(self):
        ...
        self.frame_2 = Frame(self.janela, bg='black', highlightthickness=0.5, highlightbackground='white')
        self.frame_2.place(relx=0.03, rely=0.70, relwidth=0.94, relheight=0.17)
    ...
    def botoes(self):
        ...
        self.btSearch = Button(self.frame_2, text='IMPORTAR XLSX', bg='red', command=self.importar_xls)
        self.btSearch.place(relx=0.47, rely=0.25, relwidth=0.10, relheight=0.50)
        
        self.btSearch = Button(self.frame_2, text='IMPORTAR CSV', bg='red', command=self.importar_csv)
        self.btSearch.place(relx=0.77, rely=0.25, relwidth=0.10, relheight=0.50)
    ...
    def importar_xls(self, marca):
        marca = self.cb_marcas.get().lower()
        sql = f'SELECT * from {marca}'
        dataframe = pd.DataFrame(sql)
        dataframe.to_excel('./shoes.xlsx')

    def importar_csv(self, marca):
        sql = f'SELECT * from {marca}'
        dataframe = pd.DataFrame(sql)
        dataframe.to_excel('./shoes.csv')
